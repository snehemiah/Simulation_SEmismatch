#Simulatin for chapter 3
#Model that evaluate changes in recruitment, occupancy, and mortality
...

{
  ...
  M_benchmark<-read.csv("Simulation/m_age.csv", header=F) #full year natural mortality at age
  M_markrecap<-read.csv('Simulation/Chapter3/data/estm_output.csv',header=TRUE)
...
{ #start code to simulate population
  ...
  M<-array(0,dim=c(region,tstep,mnyrs,lage), dimnames=list(region.names,tstep.names,year.names,age.names))
  ...
  true_recruit<-numeric(mnyrs)
  true_recruit_stock<-array(0,dim=c(stock,mnyrs), dimnames=list(stock.names,year.names))
  ...
  prop<-array(0,dim=c(stock,region,tstep,mnyrs,lage),dimnames =list(stock.names,region.names,tstep.names,year.names,age.names))
  percent_remain<-array(0,dim=c(stock,region,mnyrs,tstep,lage),dimnames=list(stock.names,region.names,year.names,tstep.names,age.names))
  ...
  #make a gradient based on what you think happens
  
  #time-step1
  # Initialize matrix for mortality over the years
  Move_bay1 <- matrix(0, nrow = mnyrs, ncol = lage-1) # matrix of mortality for each year and age for cb in Jan-June
  
  # Define the years for interpolation (0 to 1)
  years <- seq(1, 30, length.out = mnyrs) # assuming mnyrs corresponds to years 1990-2020
  # Define the number of years per decade
  years_per_decade <- mnyrs / 3
  
  
  start_1<-vector(length=lage-1) #year 1
  start_2<-vector(length=lage-1) #year 11
  start_3<-vector(length=lage-1) #year 21
  midpoint_1<-vector(length=lage-1) #year 6
  midpoint_2<-vector(length=lage-1) #year 16
  midpoint_3<-vector(length=lage-1) #year 26
  end_1<-vector(length=lage-1) #year 10
  end_2<-vector(length=lage-1) #year 20
  end_3<-vector(length=lage-1) #year 30
  
  
  # For each mortality rate
  for (i in 1:lage-1) {
    # Define midpoints for each decade
    midpoint_1[i] <- 0.75*prop_bay1[i] #average of first decade is 10% of occupancy probabilities
    midpoint_2[i]<- prop_bay1[i]
    midpoint_3[i]<- 1.25*prop_bay1[i]
    
    # Calculate start and end values for each decade based on midpoints
    #take half of the difference
    start_1[i] <- midpoint_1[i] - 0.5 * (midpoint_2[i] - midpoint_1[i])
    end_1[i] <- midpoint_1[i] + 0.5 * (midpoint_2[i] - midpoint_1[i])
    
    start_2[i] <- midpoint_2[i] - 0.5 * (midpoint_3[i] - midpoint_2[i])
    end_2[i] <- midpoint_2[i] + 0.5 * (midpoint_3[i] - midpoint_2[i])
    
    start_3[i] <- midpoint_3[i]- 0.5 * (midpoint_3[i] - midpoint_2[i])
    end_3[i] <- midpoint_3[i] + 0.5 * (midpoint_3[i] - midpoint_2[i]) # avoid same as midpoint
    
    # Ensure that the starting and ending values do not go below zero
    start_1[i] <- max(start_1[i], 0)
    end_1[i] <- max(end_1[i], 0)
    start_2[i] <- max(start_2[i], 0)
    end_2[i]<- max(end_2[i], 0)
    start_3[i] <- max(start_3[i], 0)
    end_3[i] <- max(end_3[i], 0)
    
    #ensure values are not above 1
    start_1[i] <- min(start_1[i], 1)
    end_1[i] <- min(end_1[i], 1)
    start_2[i] <- min(start_2[i], 1)
    end_2[i]<- min(end_2[i], 1)
    start_3[i] <- min(start_3[i], 1)
    end_3[i] <- min(end_3[i], 1)
    
    
    # Create sequences of years for each decade
    decade_1_years <- seq(1, 10, length.out = years_per_decade)
    decade_2_years <- seq(11, 20, length.out = years_per_decade)
    decade_3_years <- seq(21, 30, length.out = years_per_decade)
  }
  for(i in 1:14){
    # Interpolate mortality values for each decade
    move_1 <- approx(c(1, 10), c(start_1[i], end_1[i]), xout = decade_1_years)$y
    move_2 <- approx(c(11, 20), c(start_2[i], end_2[i]), xout = decade_2_years)$y
    move_3 <- approx(c(21, 30), c(start_3[i], end_3[i]), xout = decade_3_years)$y
    
    # Combine the mortality values for the full period
    combined_values <- c(move_1, move_2, move_3)
    
    # Store the result in the matrix
    Move_bay1[, i] <- combined_values # The time-series of natural mortality for the Chesapeake Bay
  }
  
  
  # Move_bay1<-as.data.frame(Move_bay1)
  # colnames(Move_bay1)<-seq(2,15,by=1)
  # 
  # Move_bay1$year<-seq(1,30,by=1)
  # Moutoputnew<-Move_bay1 %>% pivot_longer(cols = 1:14, names_to = "Age", values_to = "Move")
  # Moutoputnew$Age<-as.numeric(Moutoputnew$Age)
  # Moutoputnew$Decade<-ifelse(Moutoputnew$year<=10,1,ifelse(Moutoputnew$year>=21,3,2))
  # 
  # 
  # 
  # 
  # ggplot(data=Moutoputnew)+
  #   geom_line(aes(x=Age,y=Move,col=as.factor(year)))+facet_wrap(~Decade)
  # # View(Moutoputnew)
  # # for(a in 2:lage-1){
  # #   print(mean(Moutoputnew$Move[Moutoputnew$Age==a & Moutoputnew$Decade==2]))
  # # }
  
  
  
  ##time-step 2 oocupancies for the chespaeake bay in jul-dec
  
  # Initialize matrix for mortality over the years
  Move_bay2 <- matrix(0, nrow = mnyrs, ncol = lage-1) # matrix of mortality for each year and age
  ...
  prop_bay2_dec1<-prop_bay2
  
  
  #average for second 10 years
  age2<-seq(7,14,by=1)
  prop_bay2_dec2<-vector(length=lage-1) #hold mid-point of probabilitioes in year 11-20
  prop_bay2_dec2[1:7]<-prop_bay2[1:7] #ages 2-11
  start2<-prop_bay2_dec2[7]
  #approximate for age 12-15+
  val2<-vector(length=length(age2))
  for(a in 1:8){
    val2[a]<-approx(c(7,14),c(start2,1),xout=age2[a])$y
  }
  prop_bay2_dec2[8:14]<-val2[2:8] #setting values for ages 12-15
  
  
  #average for last 10 years
  
  #approximate for age 10-14, age 15 still at 1
  age3<-seq(7,12,by=1)
  prop_bay2_dec3<-vector(length=lage-1)
  prop_bay2_dec3[1:7]<-prop_bay2[1:7] #ages 2-11
  start3<-prop_bay2_dec2[7]
  val3<-vector(length=length(age3))
  for(a in 1:6){
    val3[a]<-approx(c(7,12),c(start3,1),xout=age3[a])$y
  }
  prop_bay2_dec3[8:12]<-val3[2:6] #setting values for ages 10-14
  prop_bay2_dec3[13:14]<-1 #set last age to 1 as well
  
  
  start_1_t2<-vector(length=lage-1) #year 1
  start_2_t2<-vector(length=lage-1) #year 11
  start_3_t2<-vector(length=lage-1) #year 21
  midpoint_1_t2<-vector(length=lage-1) #year 6
  midpoint_2_t2<-vector(length=lage-1) #year 16
  midpoint_3_t2<-vector(length=lage-1) #year 26
  end_1_t2<-vector(length=lage-1) #year 10
  end_2_t2<-vector(length=lage-1) #year 20
  end_3_t2<-vector(length=lage-1) #year 30
  
  
  # For each mortality rate
  for (i in 1:lage-1) {
    # Define midpoints for each decade
    midpoint_1_t2[i] <- 0.75*prop_bay2_dec1[i] #average of first decade is 10% of occupancy probabilities
    midpoint_2_t2[i]<- prop_bay2_dec2[i]
    midpoint_3_t2[i]<- 1.25*prop_bay2_dec3[i]
    
    # Calculate start and end values for each decade based on midpoints
    #take half of the difference
    #start_1_t2[i] <- midpoint_1_t2[i] - 0.5 * (midpoint_2_t2[i] - midpoint_1_t2[i])
    start_1_t2[i] <-  midpoint_1_t2[i] -0.5 * (midpoint_2_t2[i] - midpoint_1_t2[i])
    end_1_t2[i] <- midpoint_1_t2[i] + 0.5 * (midpoint_2_t2[i] - midpoint_1_t2[i])
    
    start_2_t2[i] <- midpoint_2_t2[i] - 0.5 * (midpoint_3_t2[i] - midpoint_2_t2[i])
    end_2_t2[i] <- midpoint_2_t2[i] + 0.5 * (midpoint_3_t2[i] - midpoint_2_t2[i])
    
    start_3_t2[i] <- midpoint_3_t2[i]- 0.5 * (midpoint_3_t2[i] - midpoint_2_t2[i])
    end_3_t2[i] <- midpoint_3_t2[i] + 0.5 * (midpoint_3_t2[i] - midpoint_2_t2[i]) # avoid same as midpoint
    # }
    # for(i in 11:14){
    #   start_1_t2[i]<-ifelse(start_1_t2[i]<start_1_t2[i-1],start_1_t2[i-1],start_1_t2[i])
    #   start_2_t2[i]<-ifelse(start_2_t2[i]<start_2_t2[i-1],start_2_t2[i-1],start_2_t2[i])
    #   start_3_t2[i]<-ifelse(start_3_t2[i]<start_3_t2[i-1],start_3_t2[i-1],start_3_t2[i])
    #   
    #   
    # }
    # for (i in 1:lage-1) {
    #   
    # Ensure that the starting and ending values do not go below zero or below previous timestep
    start_1_t2[i] <- max(start_1_t2[i], 0)
    end_1_t2[i] <- max(end_1_t2[i], 0)
    start_2_t2[i] <- max(start_2_t2[i], end_1_t2[i]) #making sure isn't below previous time-step
    end_2_t2[i]<- max(end_2_t2[i], 0)
    start_3_t2[i] <- max(start_3_t2[i], end_2_t2[i])#making sure isn't below previous time-step
    end_3_t2[i] <- max(end_3_t2[i], 0)
    
    #ensure values are not above 1
    start_1_t2[i] <- min(start_1_t2[i], 1)
    end_1_t2[i] <- min(end_1_t2[i], 1)
    start_2_t2[i] <- min(start_2_t2[i], 1)
    end_2_t2[i]<- min(end_2_t2[i], 1)
    start_3_t2[i] <- min(start_3_t2[i], 1)
    end_3_t2[i] <- min(end_3_t2[i],1)
  ...
  for(i in 1:14){
    # Interpolate mortality values for each decade
    move_1_t2 <- approx(c(1, 10), c(start_1_t2[i], end_1_t2[i]), xout = decade_1_years)$y
    move_2_t2 <- approx(c(11, 20), c(start_2_t2[i], end_2_t2[i]), xout = decade_2_years)$y
    move_3_t2 <- approx(c(21, 30), c(start_3_t2[i], end_3_t2[i]), xout = decade_3_years)$y
    
    # Combine the mortality values for the full period
    combined_values_t2 <- c(move_1_t2, move_2_t2, move_3_t2)
    
    # Store the result in the matrix
    Move_bay2[, i] <- combined_values_t2 # The time-series of natural mortality for the Chesapeake Bay
  }
  
  for(y in 1:mnyrs){
    for(a in 8:14){
      if(Move_bay2[y,a-1]>Move_bay2[y,a]){
        Move_bay2[y,a]=Move_bay2[y,a-1]
      }
    }
  }
  #  Move_bay2<-as.data.frame(Move_bay2)
  # colnames(Move_bay2)<-seq(2,15,by=1)
  # 
  # Move_bay2$year<-seq(1,30,by=1)
  # Moutoputnew_t2<-Move_bay2 %>% pivot_longer(cols = 1:14, names_to = "Age", values_to = "Move")
  # Moutoputnew_t2$Age<-as.numeric(Moutoputnew_t2$Age)
  # Moutoputnew_t2$Decade<-ifelse(Moutoputnew_t2$year<=10,1,ifelse(Moutoputnew_t2$year>=21,3,2))
  # 
  # Moutoputnew$timestep<-1
  # Moutoputnew_t2$timestep<-2
  # tot_prop<-rbind(Moutoputnew,Moutoputnew_t2)
  # ggplot(data=tot_prop)+
  # geom_line(aes(x=Age,y=Move,col=as.factor(year)))+facet_wrap(~timestep+Decade)#+
  
  
  
  
  ####----- now calculate percent remain in each time-step
  
  
  # Initialize matrix for mortality over the years
  remain_cb <- matrix(0, nrow = mnyrs, ncol = tstep) # matrix of mortality for each year and age
  remain_ac <- matrix(0, nrow = mnyrs, ncol = tstep) # matrix of mortality for each year and age
  
  # Define the years for interpolation (0 to 1)
  years <- seq(1, 30, length.out = mnyrs) # assuming mnyrs corresponds to years 1990-2020
  
  start_percent_cb<-c(1,1,1,1,1,1)
  end_percent_cb<-c(1,0.9,0.8,1,1,1)
  
  start_percent_ac<-c(1,1,1,1,1,1)
  end_percent_ac<-c(1,1,1,1,1,0.75)
  
  # For each mortality rate
  for (i in 1:tstep) {
    
    # Interpolate mortality values for each decade
    remaining_cbregion <- approx(c(1, 30), c(start_percent_cb[i], end_percent_cb[i]), xout = years)$y
    remaining_acregion <- approx(c(1, 30), c(start_percent_ac[i], end_percent_ac[i]), xout = years)$y
    
    # Combine the mortality values for the full period
    #combined_remain <- c(move_1_t2, move_2_t2, move_3_t2)
    
    # Store the result in the matrix
    remain_cb[, i] <- remaining_cbregion # The time-series of natural mortality for the Chesapeake Bay
    remain_ac[, i] <- remaining_acregion # The time-series of natural mortality for the Chesapeake Bay
  }
  
  
  
  ##model so that percent remain only applies to ages 8+
  # calculate percent remain
  percent_remain[,,1:30,,1]<-1 #all fish in the first time-step remain in the region they are in
  percent_remain[2,,1:30,,]<-1 #All AC stock fish never change movement
  percent_remain[1,,1:30,,1:8]<-1 #CB fish age 1-7 follow the occupancies
  
  for(y in 1:mnyrs){
    for(ts in 1:tstep){
      percent_remain[cb,cb,y,ts,8:lage]<-remain_cb[y,ts]
      percent_remain[cb,ac,y,ts,8:lage]<-remain_ac[y,ts]
  ...
  for(ts in 1:tstep){
    prop[cb,cb,ts,1:30,1]=1 #cb  stock in region 1, age 1, timestep 1 and 1
    prop[cb,ac,ts,1:30,1]=0 #cb  stock in region 2, age 1, timestep 1 an d2
    prop[ac,cb,ts,1:30,1]=0 #ac  stock in region 1, age 1, timestep 1 an d2
    prop[ac,ac,ts,1:30,1]=1 #ac  stock in region 2, age 1, timestep 1 an d2
  }
  ...
  #starting parameters for occupancy probabilities
  #first for the atlantic coast stock since those don't change
  
  prop[ac,ac,1:3,,2:15]<-prop_coast1 #occupancy probabilities for the ac stock in the coast, jan-june
  prop[ac,cb,1:3,,2:15]<-1-prop_coast1 #occupancy probabilities for the ac stock in the ches bay, jan-june
  
  prop[ac,ac,4:6,,2:15]<-prop_coast2#occupancy probabilities for the ac stock in the coast, jul-dec
  prop[ac,cb,4:6,,2:15]<-1-prop_coast2#occupancy probabilities for the ac stock in the ches bay, jjul-dec
  
  #now set occupancy probabilities for the Chesapeake bay stock
  
  for(y in 1:mnyrs){
    for(ts in 1:3){
      for(a in 2:lage){
        prop[cb,ac,ts,y,a]<-Move_bay1[y,a-1]
        prop[cb,cb,ts,y,a]<-1-Move_bay1[y,a-1]
      }
    }
  }
  for(y in 1:mnyrs){
    for(ts in 4:6){
      for(a in 2:lage){
        prop[cb,ac,ts,y,a]<-Move_bay2[y,a-1] #movement is estimated for ages 2-15t, so need to adjust scripts so it lines up with the right estimates
        prop[cb,cb,ts,y,a]<-1-Move_bay2[y,a-1]
      }
    }
  }
  
  #now calculate % remain for each timestep and through time
  occ<-array(0,dim=c(stock,region,tstep,mnyrs,lage),dimnames =list(stock.names,region.names,tstep.names,year.names,age.names))
  
  for(a in 1:lage){
    for(s in 1:stock){
      for(y in 1:mnyrs){ #years
        for(ts in 1:tstep){
          #timestep2
          occ[s,cb,ts,y,a]<-prop[s,cb,ts,y,a]*percent_remain[s,cb,y,ts,a]+prop[s,ac,ts,y,a]*(1-percent_remain[s,ac,y,ts,a])
          occ[s,ac,ts,y,a]<-prop[s,ac,ts,y,a]*percent_remain[s,ac,y,ts,a]+prop[s,cb,ts,y,a]*(1-percent_remain[s,cb,y,ts,a])
        }
      }
    }
  }
  ...
  #organize natural mortality calculations
  head(M_markrecap)
  #M for the ocean region is jsut one estimate over time
  M_2<-subset(M_markrecap,region=='ocean')
  M_ocean<-vector(length=15)
  M_ocean[1]<-M_benchmark[1,1]
  M_ocean[2:11]<-M_2$exp_m
  M_ocean[12:15]<-M_ocean[11]
  
  #M for the chesapeake bay region was estimated for 3 different decades
  M_1_1990<-subset(M_markrecap,region=='bay' & decage==1) #first decage
  M_1_2000<-subset(M_markrecap,region=='bay' & decage==2) #first decage
  M_1_2010<-subset(M_markrecap,region=='bay' & decage==3) #first decage
  M_1<-array(0,dim=c(3,lage))
  M_1[1:3,1]<-M_benchmark[1,1]#setting first age = to M from bench mark since there is no est for age1
  M_1[1,2:11]<-M_1_1990$exp_m
  M_1[2,2:11]<-M_1_2000$exp_m
  M_1[3,2:11]<-M_1_2010$exp_m
  M_1[1,12:15]<-M_1[1,11]
  M_1[2,12:15]<-M_1[2,11]
  M_1[3,12:15]<-M_1[3,11]
  
  
  # Initialize matrix for mortality over the years
  M_bay <- matrix(0, nrow = mnyrs, ncol = lage) # matrix of mortality for each year and age
  
  # Define the years for interpolation (0 to 1)
  years <- seq(1990, 2020, length.out = mnyrs) # assuming mnyrs corresponds to years 1990-2020
  # Define the number of years per decade
  years_per_decade <- mnyrs / 3
  
  
  #create vectors to store start, middle, and end points of each decade
  
  start_1990<-vector(length=lage) #year 1
  start_2000<-vector(length=lage) #year 11
  start_2010<-vector(length=lage) #year 21
  midpoint_1990<-vector(length=lage) #year 6
  midpoint_2000<-vector(length=lage) #year 16
  midpoint_2010<-vector(length=lage) #year 26
  end_1990<-vector(length=lage) #year 10
  end_2000<-vector(length=lage) #year 20
  end_2010<-vector(length=lage) #year 30
  
  #difference_9000<-(M_1[2,]-M_1[1,])/2
  
  # For each mortality rate
  for (i in 1:lage) {
    # Define midpoints for each decade
    midpoint_1990[i] <- M_1[1, i]
    midpoint_2000[i]<- M_1[2, i]
    midpoint_2010[i]<- M_1[3, i]
    
    # Calculate start and end values for each decade based on midpoints
    #take half of the difference
    start_1990[i] <- midpoint_1990[i] - 0.5 * (midpoint_2000[i] - midpoint_1990[i])
    end_1990[i] <- midpoint_1990[i] + 0.5 * (midpoint_2000[i] - midpoint_1990[i])
    
    start_2000[i] <- midpoint_2000[i] - 0.5 * (midpoint_2010[i] - midpoint_2000[i])
    end_2000[i] <- midpoint_2000[i] + 0.25 * (midpoint_2010[i] - midpoint_2000[i])
    
    start_2010[i] <- midpoint_2010[i]- 0.5 * (midpoint_2010[i] - midpoint_2000[i])
    end_2010[i] <- midpoint_2010[i] + 0.5 * (midpoint_2010[i] - midpoint_2000[i]) # avoid same as midpoint
    
    # Ensure that the starting and ending values do not go below zero
    start_1990[i] <- max(start_1990[i], 0)
    end_1990[i] <- max(end_1990[i], 0)
    start_2000[i] <- max(start_2000[i], 0)
    end_2000[i]<- max(end_2000[i], 0)
    start_2010[i] <- max(start_2010[i], 0)
    end_2010[i] <- max(end_2010[i], 0)
    
    #make sure values are never half of the average
    if (start_1990[i]<0.5*midpoint_1990[i]) {
      start_1990[i]<-0.5*midpoint_1990[i]
    }
    if (start_2000[i]<0.5*midpoint_2000[i]) {
      start_2000[i]<-0.5*midpoint_2000[i]
    }
    if (start_2010[i]<0.5*midpoint_2010[i]) {
      start_2010[i]<-0.5*midpoint_2010[i]
    }
    #make sure values are never above 50% increase of the average
    if (end_1990[i]>1.5*midpoint_1990[i]) {
      end_1990[i]<-1.5*midpoint_1990[i]
    }
    if (end_2000[i]>1.5*midpoint_2000[i]) {
      end_2000[i]<-1.5*midpoint_2000[i]
    }
    if (end_2010[i]>1.5*midpoint_2010[i]) {
      end_2010[i]<-1.5*midpoint_2010[i]
    }
    # Create sequences of years for each decade
    decade_1990_years <- seq(1990, 1999, length.out = years_per_decade)
    decade_2000_years <- seq(2000, 2009, length.out = years_per_decade)
    decade_2010_years <- seq(2010, 2019, length.out = years_per_decade)
    
    # Interpolate mortality values for each decade
    mortality_1990 <- approx(c(1990, 1999), c(start_1990[i], end_1990[i]), xout = decade_1990_years)$y
    mortality_2000 <- approx(c(2000, 2009), c(start_2000[i], end_2000[i]), xout = decade_2000_years)$y
    mortality_2010 <- approx(c(2010, 2019), c(start_2010[i], end_2010[i]), xout = decade_2010_years)$y
    
    # Combine the mortality values for the full period
    combined_values <- c(mortality_1990, mortality_2000, mortality_2010)
    
    # Store the result in the matrix
    M_bay[, i] <- combined_values # The time-series of natural mortality for the Chesapeake Bay
  }
  
  # for(a in 1:15){
  #   print(mean(M_bay[1:10,a]))
  # }
  #plot(M_bay[1,],pch=16,ylim=c(0,1.2))
  #points(M_bay[30,],pch=16,col="red")
  
  # # #calculate M for both regions in 2-month time-steps
  for(ts in 1:tstep){
    for(a in 1:lage){
      M[2,ts,1:30,a]<-(1/6)*M_ocean[a] #set natural mortality to the same for all years in the ocean region
      for(y in 1:mnyrs){
        #testing how model runs with same m over time but varying by region
        M[1,ts,y,a]<-(1/6)*M_bay[y,a] #set natural mortality for the bay to be changing over time
      }
    }
  ...
  # for(ts in 1:tstep){
  #   for(a in 1:lage){
  #     #testing if model works with same m for both regions for all year
  #     M[2,ts,1:30,a]<-(1/6)*M_ocean[a] #set natural mortality to the same for all years in the ocean region
  #     #testing how model runs with same m over time but varying by region
  #     M[1,ts,1:30,a]<-(1/6)*M_1[3,a] #set natural mortality for the bay to be changing over time
  #   }
  # }#close time step loop
  
  #testing model to see if it's set up correctly:
  # for(r in 1:region){
  #   for(ts in 1:tstep){
  #     for(y in 1:mnyrs){
  #       for(a in 1:lage){
  #         M[r,ts,y,a]<-(1/6)*M_benchmark[1,a]
  #       }
  #     }
  #   }
  # }
  ...
  #setting mean F for all timesteps in both regions
    ...
    for(ts in 1:tstep){
      for(y in 1:mnyrs){
        for(a in fage:lage){
          ...
          Fmort[r,ts,y,a]<-F_3step[r,ts,y,a] #multiple fsel by calculation to get total F for each time-step
        } #close age loop
      }#close year loop
    }#close tstep loop
  } #close region loop
    ...
    for(ts in 1:tstep){
      for(y in 1:mnyrs){
        for(a in fage:lage){
          ...
          Z[r,ts,y,a]<-Fmort[r,ts,y,a]+M[r,ts,y,a] #M is constant across each timestep so divide annual M by 1/6
        }#close age loop
      }#close time step loop
    }#clopse year loop
  }#close stock
  ...
  #recruitment
  log_R_year<-(array(0,dim=c(stock,mnyrs),dimnames = list(stock.names,year.names)))
  
  #ac stock recruitment is stable overtime
  log_R_year[2,]<-log_R[2]
  #cb stock recruitment has gradual decline overtime and ends at the same log_r as the ac
  #testing changes to productivity overtime
  starting_r<-log_R[1]
  ending_r<-log_R[2]
  fullyrs<-seq(1,mnyrs,by=1)
  val<-vector(length=length(mnyrs))#set vector to hold interpolation
  for(m in 1:mnyrs){
    val[m]<-approx(c(1,mnyrs),c(starting_r,ending_r),xout=fullyrs[m])$y
  }
  log_R_year[1,]<-val #set log_r to new values calculated for interpolation
  #log_R_year[1,]<-log_R[1] #setting values to base r to test model is running
  #log_R_year
  
  
  #Caclulate Nt and N at age in the first year and first timestep
  for(s in 1:stock){
    recruit[s,1]<-exp(log_R_year[s,1])
  ...
  for(s in 1:stock){
    for(r in 1:region){
      for(a in 2:lage){
        ...
        N_eq[s,a]<-Nt[s,fmyr,1,a-1]*exp(-(M_benchmark[1,a-1]+Feq[s,a-1]))#first year, age =a, 1=ts1
      ...
      Nt[s,fmyr,1,lage]<-Nt[s,fmyr,1,lage]/(1-exp(-(M_benchmark[1,a-1]+Feq[s,lage]))) #calculation of the plus group in the first year
  ...
  for(s in 1:stock){
    for(r in 1:region){
      for(a in 1:lage){
        N[s,r,fmyr,1,a]<-Nt[s,fmyr,1,a]*occ[s,r,1,fmyr,a]
      }#close age loop
    }#close region loop
  }#close stock loop
  #range(N)
  ...
  for(s in 1:stock){
    for(a in 1:lage){
      ...
      # N[s,cb,fmyr,2,a]<- N[s,cb,fmyr,1,a]*exp(-Z[cb,1,fmyr,a])*percent_remain[s,cb,fmyr,2,a]+N[s,ac,fmyr,1,a]*exp(-Z[ac,1,fmyr,a])*(1-percent_remain[s,ac,fmyr,2,a])#calculating total abundance for each stock in the Bay as the total of fish that stay in the bay + the fish that leave the AC
      # N[s,ac,fmyr,2,a]<- N[s,ac,fmyr,1,a]*exp(-Z[ac,1,fmyr,a])*percent_remain[s,ac,fmyr,2,a]+N[s,cb,fmyr,1,a]*exp(-Z[cb,1,fmyr,a])*(1-percent_remain[s,cb,fmyr,2,a])#ccalculating total abundance for each stock in the ocean as the total of fish that stay in the ocean + the fish that leave the ocean
  ...
  for(s in 1:stock){
    for(r in 1:region){
      for(a in 1:lage){
        N[s,r,fmyr,2,a]<-Nt[s,fmyr,2,a]*occ[s,r,2,fmyr,a]
      }#close age loop
    }#close region loop
  }#close stock loop
  #N[,,fmyr,,]
  ...
  for(s in 1:stock){
    for(a in 1:lage){
      ...
      #need to add region to the percent remain and then add 1-%*N
      # N[s,cb,fmyr,3,a]<- N[s,cb,fmyr,2,a]*exp(-Z[cb,2,fmyr,a])*percent_remain[s,cb,fmyr,3,a]+N[s,ac,fmyr,2,a]*exp(-Z[ac,2,fmyr,a])*(1-percent_remain[s,ac,fmyr,3,a])#calculating total abundance for each stock in the Bay as the total of fish that stay in the bay + the fish that leave the AC
      # N[s,ac,fmyr,3,a]<- N[s,ac,fmyr,2,a]*exp(-Z[ac,2,fmyr,a])*percent_remain[s,ac,fmyr,3,a]+N[s,cb,fmyr,2,a]*exp(-Z[cb,2,fmyr,a])*(1-percent_remain[s,cb,fmyr,3,a])#ccalculating total abundance for each stock in the ocean as the total of fish that stay in the ocean + the fish that leave the ocean
  ...
  for(s in 1:stock){
    for(r in 1:region){
      for(a in 1:lage){
        N[s,r,fmyr,3,a]<-Nt[s,fmyr,3,a]*occ[s,r,3,fmyr,a]
      }#close age loop
    }#close region loop
  }#close stock loop
  ...
  for(s in 1:stock){
    for(r in 1:region){
      for(a in 1:lage){
        ...
        N[s,r,fmyr,4,a]<-Nt[s,fmyr,4,a]*occ[s,r,4,fmyr,a]
      }
    }
  }
  ...
  for(s in 1:stock){
    for(a in 1:lage){
      ...
      #need to add region to the percent remain and then add 1-%*N
      # N[s,cb,fmyr,5,a]<- N[s,cb,fmyr,4,a]*exp(-Z[cb,4,fmyr,a])*percent_remain[s,cb,fmyr5,a]+ N[s,ac,fmyr,4,a]*exp(-Z[ac,4,fmyr,a])*(1-percent_remain[s,ac,fmyr,5,a])#calculating total abundance for each stock in the Bay as the total of fish that stay in the bay + the fish that leave the AC
      # N[s,ac,fmyr,5,a]<- N[s,ac,fmyr,4,a]*exp(-Z[ac,4,fmyr,a])*percent_remain[s,ac,fmyr5,a]+ N[s,cb,fmyr,4,a]*exp(-Z[cb,4,fmyr,a])*(1-percent_remain[s,cb,fmyr,5,a])#ccalculating total abundance for each stock in the ocean as the total of fish that stay in the ocean + the fish that leave the ocean
  ...
  for(s in 1:stock){
    for(r in 1:region){
      for(a in 1:lage){
        #put each stock in the correct spatial region
        N[s,r,fmyr,5,a]<-Nt[s,fmyr,5,a]*occ[s,r,5,fmyr,a]
      }
    }
  }
  #time step 6
  for(s in 1:stock){
    for(a in 1:lage){
      ...
      #need to add region to the percent remain and then add 1-%*N
      # N[s,cb,fmyr,6,a]<- N[s,cb,fmyr,5,a]*exp(-Z[cb,5,fmyr,a])*percent_remain[s,cb,fmyr,6,a]+ N[s,ac,fmyr,5,a]*exp(-Z[ac,5,fmyr,a])*(1-percent_remain[s,ac,fmyr,6,a])#calculating total abundance for each stock in the Bay as the total of fish that stay in the bay + the fish that leave the AC
      # N[s,ac,fmyr,6,a]<- N[s,ac,fmyr,5,a]*exp(-Z[ac,5,fmyr,a])*percent_remain[s,ac,fmyr,6,a]+ N[s,cb,fmyr,5,a]*exp(-Z[cb,5,fmyr,a])*(1-percent_remain[s,cb,fmyr,6,a])#ccalculating total abundance for each stock in the ocean as the total of fish that stay in the ocean + the fish that leave the ocean
  ...
  for(s in 1:stock){
    for(r in 1:region){
      for(a in 1:lage){
        #put each stock in the correct spatial region
        N[s,r,fmyr,6,a]<-Nt[s,fmyr,6,a]*occ[s,r,6,fmyr,a]
      }
    }
  }
  #N[1,,fmyr,,]
  ...
  for(y in 2:mnyrs){
    ...
    t6=6
    for(s in 1:stock){
      recruit[s,y]<-exp(log_R_year[s,y]+log_Rdevs[s,y]) ##recritment is in ts 1 for now, this may be better in another time-step
      ...
      for(r in 1:region){
        N[s,r,y,t1,]<-Nt[s,y,t1,]*occ[s,r,t1,y,]
      }# close region loop
      ...
      for(a in fage:lage){
        ...
        # N[s,cb,y,t2,a]<- N[s,cb,y,t1,a]*exp(-Z[cb,t1,y,a])*percent_remain[s,cb,y,t2,a]+ N[s,ac,y,t1,a]*exp(-Z[ac,t1,y,a])*(1-percent_remain[s,ac,y,t2,a])#calculating total abundance for each stock in the Bay as the total of fish that stay in the bay + the fish that leave the AC
        # N[s,ac,y,t2,a]<- N[s,ac,y,t1,a]*exp(-Z[ac,t1,y,a])*percent_remain[s,ac,y,t2,a]+ N[s,cb,y,t1,a]*exp(-Z[cb,t1,y,a])*(1-percent_remain[s,cb,y,t2,a])#ccalculating total abundance for each stock in the ocean as the total of fish that stay in the ocean + the fish that leave the ocean
        # Nt[s,y,t2,a]<-sum(N[s,,y,t2,a]) #keeping track of total stock abundnace in Mar/apr
      } #close age loop
      #put fish in each of the regions
      for(r in 1:region){
        N[s,r,y,t2,]<-Nt[s,y,t2,]*occ[s,r,t2,y,]
      }# close region loop
      ...
      for(a in fage:lage){
        ...
        # N[s,cb,y,t3,a]<- N[s,cb,y,t2,a]*exp(-Z[cb,t2,y,a])*percent_remain[s,cb,y,t3,a]+N[s,ac,y,t2,a]*exp(-Z[ac,t2,y,a])*(1-percent_remain[s,ac,y,t3,a])#calculating total abundance for each stock in the Bay as the total of fish that stay in the bay + the fish that leave the AC
        # N[s,ac,y,t3,a]<- N[s,ac,y,t2,a]*exp(-Z[ac,t2,y,a])*percent_remain[s,ac,y,t3,a]+ N[s,cb,y,t2,a]*exp(-Z[cb,t2,y,a])*(1-percent_remain[s,cb,y,t3,a])#ccalculating total abundance for each stock in the ocean as the total of fish that stay in the ocean + the fish that leave the ocean
        # Nt[s,y,t3,a]<-sum(N[s,,y,t3,a]) #keeping track of total stock abundnace in may/june
      } #close age loop
      #put fish in each of the regions
      for(r in 1:region){
        N[s,r,y,t3,]<-Nt[s,y,t3,]*occ[s,r,t3,y,]
      }# close region loop
      ...
      for(r in 1:region){
        N[s,r,y,t4,]<-Nt[s,y,t4,]*occ[s,r,t4,y,]
      }# close region loop
      ...
      for(a in fage:lage){
        ...
        # N[s,cb,y,t5,a]<- N[s,cb,y,t4,a]*exp(-Z[cb,t4,y,a])*percent_remain[s,cb,y,t5,a]+ N[s,ac,y,t4,a]*exp(-Z[ac,t4,y,a])*(1-percent_remain[s,ac,y,t5,a])#calculating total abundance for each stock in the Bay as the total of fish that stay in the bay + the fish that leave the AC
        # N[s,ac,y,t5,a]<- N[s,ac,y,t4,a]*exp(-Z[ac,t4,y,a])*percent_remain[s,ac,y,t5,a]+ N[s,cb,y,t4,a]*exp(-Z[cb,t4,y,a])*(1-percent_remain[s,cb,y,t5,a])#calculating total abundance for each stock in the ocean as the total of fish that stay in the ocean + the fish that leave the ocean
        # Nt[s,y,t5,a]<-sum(N[s,,y,t5,a]) #keeping track of total stock abundnace in Mar/apr
      } #close age loop
      for(r in 1:region){
        N[s,r,y,t5,]<-Nt[s,y,t5,]*occ[s,r,t5,y,]
      }# close region loop
      ...
      for(a in fage:lage){
        ...
        # N[s,cb,y,t6,a]<- N[s,cb,y,t5,a]*exp(-Z[cb,t5,y,a])*percent_remain[s,cb,y,t6,a]+ N[s,ac,y,t5,a]*exp(-Z[ac,t5,y,a])*(1-percent_remain[s,ac,y,t6,a])#calculating total abundance for each stock in the Bay as the total of fish that stay in the bay + the fish that leave the AC
        # N[s,ac,y,t6,a]<- N[s,ac,y,t5,a]*exp(-Z[ac,t5,y,a])*percent_remain[s,ac,y,t6,a]+ N[s,cb,y,t5,a]*exp(-Z[cb,t5,y,a])*(1-percent_remain[s,cb,y,t6,a])#ccalculating total abundance for each stock in the ocean as the total of fish that stay in the ocean + the fish that leave the ocean
        # Nt[s,y,t6,a]<-sum(N[s,,y,t6,a]) #keeping track of total stock abundnace in Mar/apr
      } #clos eage loop
      for(r in 1:region){
        N[s,r,y,t6,]<-Nt[s,y,t6,]*occ[s,r,t6,y,]
      }# close region loop
  ...
  for(y in 1:mnyrs){
    ...
    true_recruit[y]<-sum(Nt[,y,1,fage])
  ...
  # for(s in 1:stock){
  #   R[s]<-exp(log_R_year[s,30])/sum(exp(log_R_year[,30]))
  ...
  #     for(ts in 1:tstep){
  #       for(a in fage:lage){
  #         Z_eq[r,ts,a]<-Fstar[r,ts,a]+M[r,ts,30,a]
  #       }#close age loop
  #     }# close tstep loop
  #   }#close region loop
  #   
  ...
  #       for(r in 1:region){
  #         for(a in 1:lage){
  #           Nf[s,r,ts1,y,a]<-Nf_stock[s,ts1,y,a]*occ[s,r,ts1,30,a] #set occupancy to the last year to get reference point
  #         }#close age loop
  #       }#close region loop
  #     }#close stock loop
  #     
  ...
  #       for(r in 1:region){
  #         for(a in 1:lage){
  #           Nf[s,r,ts2,y,a]<-Nf_stock[s,ts2,y,a]*occ[s,r,ts2,30,a] #
  #         }#close age loop
  #       }#close region loop
  #     }#close stock loop
  #     
  ...
  #       for(r in 1:region){
  #         for(a in 1:lage){
  #           Nf[s,r,ts3,y,a]<-Nf_stock[s,ts3,y,a]*occ[s,r,ts3,30,a] #
  #         }#close age loop
  #       }#close region loop
  #     }#close stock loop
  #     
  ...
  #         for(a in 1:lage){
  #           #put each stock in the correct spatial region
  #           Nf[s,r,ts4,y,a]<-Nf_stock[s,ts4,y,a]*occ[s,r,ts4,30,a]
  #         }
  #       }
  #     }
  #     
  ...
  #         for(a in 1:lage){
  #           #put each stock in the correct spatial region
  #           Nf[s,r,ts5,y,a]<-Nf_stock[s,ts5,y,a]*occ[s,r,ts5,30,a]
  #         }
  #       }
  #     }
  #     
  ...
  #         for(a in 1:lage){
  #           #put each stock in the correct spatial region
  #           Nf[s,r,ts6,y,a]<-Nf_stock[s,ts6,y,a]*occ[s,r,ts6,30,a]
  #         }
  #       }
  #     }
  #   } #close year loop
  ...
  #     for(ts in 1:tstep){
  #       for(a in fage:lage){
  #         Fbar_stock[s,a]<-Fbar_stock[s,a]+Fbar[r,ts,a]*occ[s,r,ts,30,a] 
  #       } #close age loop
  #     } #close tstep loop
  #   }#close region loop
  # }#close stock loop
...
{ #start simdat
  ...
  #for(ts in 1:tstep){
  for(a in 2:lage){
    occ_prob_dat[cb,1,a]<- start_log_prop_bay1[a-1] 
    occ_prob_dat[cb,2,a]<- start_log_prop_bay2[a-1] 
    occ_prob_dat[ac,1,a]<- start_log_prop_coast1[a-1] 
    occ_prob_dat[ac,2,a]<- start_log_prop_coast2[a-1] 
...

{
  ...
  setwd("C:/Users/Anyone/Documents/PhD - UMCES/NCBO Project/Striped-Bass-SCAA/Rcode/Simulation/Chapter3/all_om5")
...
output_faa2_om5<- read.csv('C:/Users/Anyone/Documents/PhD - UMCES/NCBO Project/Striped-Bass-SCAA/Rcode/Simulation/Chapter3/all_om5/faamodel2/sim_results.txt',header=F, sep="")
output_se4_om5<- read.csv('C:/Users/Anyone/Documents/PhD - UMCES/NCBO Project/Striped-Bass-SCAA/Rcode/Simulation/Chapter3/all_om5/semodel4/sim_results.txt',header=F, sep="")
output_se6_om5<- read.csv('C:/Users/Anyone/Documents/PhD - UMCES/NCBO Project/Striped-Bass-SCAA/Rcode/Simulation/Chapter3/all_om5/semodel6/sim_results.txt',header=F, sep="")



output_faa2_om5$model<-"FAA"
colnames(output_faa2_om5) <- c("simnum","year","F8est","Fre","estNtot","Nre",'estb','Bre','estssb','SSBre','estF40','F40RE', 'estR','Rre' ,"obj","model")

output_se4_om5$model<-"SE" #SE4 is spatially explicit with aging error
colnames(output_se4_om5) <- c("simnum","year","F8est","Fre","estNtot","Nre","estNtotCB","NreCB","estNtotAC","NreAC",'estb','Bre','estssb','SSBre','estF40','F40RE', 'estF40_cb','F40RE_cb','estF40_ac','F40RE_ac','estR','Rre' ,'estRcb','Rre_cb','estRac','Rre_ac',"obj","model")

output_se6_om5$model<-"SE2" #SE5 is spatially explicit with stock in data 
colnames(output_se6_om5) <- c("simnum","year","F8est","Fre","estNtot","Nre","estNtotCB","NreCB","estNtotAC","NreAC",'estb','Bre','estssb','SSBre','estF40','F40RE', 'estF40_cb','F40RE_cb','estF40_ac','F40RE_ac','estR','Rre' ,'estRcb','Rre_cb','estRac','Rre_ac',"obj","model")

# ggplot()+
#   geom_boxplot(data=output_faa2, aes(x=(year),y=Nre,group=year))+
...
#   geom_boxplot(data=output_faa2, aes(x=(year),y=Fre,group=year),col="orange")+
...
# ggplot()+
#   geom_boxplot(data=output_se4_om3, aes(x=(year),y=Nre,group=year))+
...
#   geom_boxplot(data=output_se4_om3, aes(x=(year+0.75),y=Fre,group=year),col="orange")+
#   geom_hline(yintercept=0,linetype="dashed",col="red")+
#   #geom_line(data=Noutput,aes(x=year,y=Nre,col=as.factor(simnum)))+
#   ggtitle("SE Model 4 Output om3")+#facet_wrap(~model)+
#   theme_bw()+theme(legend.position="none")#+
...
output_om5<-bind_rows(output_faa2_om5,output_se4_om5,output_se6_om5)
output_om5$OM<-"OM5"


output_om5<-subset(output_om5,obj<0.01)
#range(output$obj)

write.csv(output_om5,"C:/Users/Anyone/Documents/PhD - UMCES/NCBO Project/Striped-Bass-SCAA/Rcode/Simulation/Chapter3/Final_output/output_om5.csv")
...
Noutput<-output_om5[, (colnames(output_om5) %in% coln)]
...
SSBoutput<-output_om5[, (colnames(output_om5) %in% colssb)]
...
Boutput<-output_om5[, (colnames(output_om5) %in% colbio)]
...
Foutput<-output_om5[, (colnames(output_om5) %in% colf)]
ggplot()+
...
colterm<-c("simnum","model", "Nre","NreCB", "NreAC","Fre","F40RE","F40RE_ac","F40RE_cb","Rre","year")
#colterm<-c("simnum","model", "Nre","NreCB", "NreAC","Fre","year")
terminal<-output_om5[, (colnames(output_om5) %in% colterm)]
terminal<-terminal %>% pivot_longer(cols = c("Nre","NreCB", "NreAC","Rre","Fre","F40RE","F40RE_ac","F40RE_cb"), names_to = "Type", values_to = "RE")
...
objoutput<-output_om5[, (colnames(output_om5) %in% colobj)]
...
estfage_faa2<- read.csv('C:/Users/Anyone/Documents/PhD - UMCES/NCBO Project/Striped-Bass-SCAA/Rcode/Simulation/Chapter3/all_om5/faamodel2/sim_F_results.txt',header=F, sep="")
estfage_se4<- read.csv('C:/Users/Anyone/Documents/PhD - UMCES/NCBO Project/Striped-Bass-SCAA/Rcode/Simulation/Chapter3/all_om5/semodel4/sim_F_results.txt',header=F, sep="")
estfage_se5<- read.csv('C:/Users/Anyone/Documents/PhD - UMCES/NCBO Project/Striped-Bass-SCAA/Rcode/Simulation/Chapter3/all_om5/semodel6/sim_F_results.txt',header=F, sep="")
...
output_f_om5<-rbind(estfage_faa2,estfage_se4,estfage_se5)

output_f_om5$operating<-"OM5"
write.csv(output_f_om5,"C:/Users/Anyone/Documents/PhD - UMCES/NCBO Project/Striped-Bass-SCAA/Rcode/Simulation/Chapter3/Final_output/foutput_om5.csv")


Fage<-paste("Fest",seq(1,15,by=1),sep="") #est F
FageRE<-paste("Fre",seq(1,15,by=1),sep="")
colnames(output_f_om5) <- c("simnum","Year",Fage,FageRE,"obj","model")
output_f_om5$operating<-"OM5"
#View(output_f)
#colnames(estfage_se4) <- c("simnum","Year",Fage,FageRE,"obj","model")


ggplot(data=subset(output_f_om5))+
...
output_F_2<-output_f_om5[, (colnames(output_f_om5) %in% col_fest)]
output_F_2<-output_F_2 %>% pivot_longer(cols = Fre1:Fre15, names_to = "Age", values_to = "RE")

terminal_2<-output_f_om5[, (colnames(output_f_om5) %in% col_fest)]
