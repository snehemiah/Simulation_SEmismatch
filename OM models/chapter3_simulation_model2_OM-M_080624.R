...
{ #start code to simulate population
  ...
  #log_Rdevs<-array(rnorm(mnyrs,0,0.0), dim=c(stock, mnyrs), dimnames=list(stock.names, year.names))
  ...
  mtable_table<- melt(M)
  #head(occ_table)
  colnames(mtable_table)<-c("Region","TimeStep","Year","Age","M")
  write.csv(mtable_table,"C:/Users/Anyone/Documents/PhD - UMCES/NCBO Project/Striped-Bass-SCAA/Rcode/Simulation/Chapter3/Final_output/mort_om2.csv")
  ...
  for(r in 1:region){
    for(y in 1:mnyrs){
      for(a in fage:lage){
        F_3step[r,1:3,y,a]<-(1/3)*F_6mo[r,1,y,a]#*fsel[1,y,1,a] #divide by 3 to get 3 time-stpes for each 6-months
        F_3step[r,4:6,y,a]<-(1/3)*F_6mo[r,2,y,a]#*fsel[1,y,2,a] #divide by 3 to get 3 time-stpes for each 6-months
      }
    }
  }
  ...
  # M_eq<-array(0,dim=c(stock,lage)) #storing M in equilibrium
  # for(a in 1:lage){
  #   M_eq[1,a]<-M_bay[1,a]  #set CB M equal to first year of M in the bay
  #   M_eq[2,]<-M_ocean[a] #set AC M equal to M in ocean
  # }
  ...
  for(s in 1:stock){
    for(r in 1:region){
      for(a in 2:lage){
        ...
        #N_eq[s,a]<-Nt[s,fmyr,1,a-1]*exp(-(M_eq[s,a-1]+Feq[s,a-1]))#first year, age =a, 1=ts1
        
        Nt[s,fmyr,1,a]<-N_eq[s,a] #then plugging in abundance in the first year as the equillibrium abundance times the abundance deviations
      } #close age loop
      Nt[s,fmyr,1,lage]<-Nt[s,fmyr,1,lage]/(1-exp(-(M_benchmark[1,lage]+Feq[s,lage]))) #calculation of the plus group in the first year
      #Nt[s,fmyr,1,lage]<-Nt[s,fmyr,1,lage]/(1-exp(-(M_eq[s,lage]+Feq[s,lage]))) #calculation of the plus group in the first year
  ...
  #now calculate Nt and N in the first year, in time-step 2 
  #timestep 2
  for(s in 1:stock){
    for(a in 1:lage){
  ...
  #Calculate Nt and N in the first year and fourth time-step (july)
  for(a in 1:lage){
  ...
  for(G in 0:150){ #looping through G's
    ...
    for(r in 1:region){
      for(ts in 1:tstep){
        for(a in fage:lage){
          Z_eq[r,ts,a]<-Fstar[r,ts,a]+M[r,ts,30,a]#(1/6*M[1,a])
        }#close age loop
      }# close tstep loop
    }#close region loop
...

{
  ...
  setwd("C:/Users/Anyone/Documents/PhD - UMCES/NCBO Project/Striped-Bass-SCAA/Rcode/Simulation/Chapter3/M_om2")
...
output_faa2_om2<- read.csv('C:/Users/Anyone/Documents/PhD - UMCES/NCBO Project/Striped-Bass-SCAA/Rcode/Simulation/Chapter3/M_om2/faamodel2/sim_results.txt',header=F, sep="")
output_se4_om2<- read.csv('C:/Users/Anyone/Documents/PhD - UMCES/NCBO Project/Striped-Bass-SCAA/Rcode/Simulation/Chapter3/M_om2/semodel4/sim_results.txt',header=F, sep="")
output_se6_om2<- read.csv('C:/Users/Anyone/Documents/PhD - UMCES/NCBO Project/Striped-Bass-SCAA/Rcode/Simulation/Chapter3/M_om2/semodel6/sim_results.txt',header=F, sep="")



output_faa2_om2$model<-"FAA"
colnames(output_faa2_om2) <- c("simnum","year","F8est","Fre","estNtot","Nre",'estb','Bre','estssb','SSBre','estF40','F40RE', 'estR','Rre' ,"obj","model")

output_se4_om2$model<-"SE" #SE4 is spatially explicit with aging error
colnames(output_se4_om2) <- c("simnum","year","F8est","Fre","estNtot","Nre","estNtotCB","NreCB","estNtotAC","NreAC",'estb','Bre','estssb','SSBre','estF40','F40RE', 'estF40_cb','F40RE_cb','estF40_ac','F40RE_ac','estR','Rre' ,'estRcb','Rre_cb','estRac','Rre_ac',"obj","model")

output_se6_om2$model<-"SE2" #SE5 is spatially explicit with stock in data 
colnames(output_se6_om2) <- c("simnum","year","F8est","Fre","estNtot","Nre","estNtotCB","NreCB","estNtotAC","NreAC",'estb','Bre','estssb','SSBre','estF40','F40RE', 'estF40_cb','F40RE_cb','estF40_ac','F40RE_ac','estR','Rre' ,'estRcb','Rre_cb','estRac','Rre_ac',"obj","model")



ggplot()+
  geom_boxplot(data=output_se4_om2, aes(x=(year),y=Nre,group=year))+
  #geom_boxplot(data=output_se4_om2, aes(x=(year+0.25),y=Bre,group=year),col="blue")+
  #geom_boxplot(data=output_se4, aes(x=(year+0.5),y=SSBre,group=year),col="green")+
  geom_boxplot(data=output_se4_om2, aes(x=(year+0.75),y=Fre,group=year),col="orange")+
  geom_hline(yintercept=0,linetype="dashed",col="red")+
  #geom_line(data=Noutput,aes(x=year,y=Nre,col=as.factor(simnum)))+
  ggtitle("SE Model Abundance ")+#facet_wrap(~model)+
...
output_om2<-bind_rows(output_faa2_om2,output_se4_om2,output_se6_om2)
output_om2$OM<-"OM2"


output_om2<-subset(output_om2,obj<0.01)
#range(output$obj)

write.csv(output_om2,"C:/Users/Anyone/Documents/PhD - UMCES/NCBO Project/Striped-Bass-SCAA/Rcode/Simulation/Chapter3/Final_output/output_om2.csv")
...
print(c("FAA2",length(unique(output_om2$simnum[output_om2$model=="FAA"]))))
print(c("SE4", length(unique(output_om2$simnum[output_om2$model=="SE"]))))
print(c("SE5", length(unique(output_om2$simnum[output_om2$model=="SES"]))))

coln<-c("simnum","model", "Nre","year")
Noutput<-output_om2[, (colnames(output_om2) %in% coln)]
...
SSBoutput<-output_om2[, (colnames(output_om2) %in% colssb)]
...
Boutput<-output_om2[, (colnames(output_om2) %in% colbio)]
...
Foutput<-output_om2[, (colnames(output_om2) %in% colf)]
...
terminal<-output_om2[, (colnames(output_om2) %in% colterm)]
terminal<-terminal %>% pivot_longer(cols = c("Nre","NreCB", "NreAC","Fre","F40RE","F40RE_ac","F40RE_cb","Rre"), names_to = "Type", values_to = "RE")
...
estfage_faa2<- read.csv('C:/Users/Anyone/Documents/PhD - UMCES/NCBO Project/Striped-Bass-SCAA/Rcode/Simulation/Chapter3/M_om2/faamodel2/sim_F_results.txt',header=F, sep="")
estfage_se4<- read.csv('C:/Users/Anyone/Documents/PhD - UMCES/NCBO Project/Striped-Bass-SCAA/Rcode/Simulation/Chapter3/M_om2/semodel4/sim_F_results.txt',header=F, sep="")
estfage_se5<- read.csv('C:/Users/Anyone/Documents/PhD - UMCES/NCBO Project/Striped-Bass-SCAA/Rcode/Simulation/Chapter3/M_om2/semodel6/sim_F_results.txt',header=F, sep="")


estfage_faa2$model<-"FAA"
estfage_se4$model<-"SE" #SE4 is spatially explicit with aging error
estfage_se5$model<-"SE2" #SE5 is spatially explicit with stock in data 

output_f<-rbind(estfage_faa2,estfage_se4,estfage_se5)
output_f$OM<-"mortality"
write.csv(output_f,"C:/Users/Anyone/Documents/PhD - UMCES/NCBO Project/Striped-Bass-SCAA/Rcode/Simulation/Chapter3/Final_output/foutput_om2.csv")
