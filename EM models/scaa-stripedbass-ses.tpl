...
//Simulation Code of Spatially explicit statistical catch at age model with stock composition data
...
DATA_SECTION
  ...
  init_int fdyear  //first year for all data
  init_int ldyear  //last year for all data

  init_int fryear  //first year for regional only data
  init_int lryear  //last year for  regional onlydata

  init_int fsyear //first year for data with stock composition
  init_int lsyear //last year for data with stock composition
  ...
  //data inputs
  //first 20 years have no stock comp
  init_3darray obs_C(1,region,1,tstep,fryear,lryear)               //observed total catch for each year
  init_4darray obs_Cp(1,region,1,tstep,fryear,lryear,fage,lage)    //observed proportions at age in the catch
  init_3darray obs_CV(1,region,1,tstep,fryear,lryear)              //coefficient of variation
  //last 10 years have stock composition data
  init_4darray obs_C_stock(1,stock,1,region,1,tstep,fsyear,lsyear)               //observed total catch for each year
  init_5darray obs_Cp_stock(1,stock,1,region,1,tstep,fsyear,lsyear,fage,lage)    //observed proportions at age in the catch
  init_4darray obs_CV_stock(1,stock,1,region,1,tstep,fsyear,lsyear)              //coefficient of variation


  //spatial observed index of abundance
  init_matrix obs_I_bay(1,tstep,fryear,lryear)
  init_matrix obs_I_coast(1,tstep,fryear,lryear)
  init_3darray obs_I_bay_stock(1,stock,1,tstep,fsyear,lsyear)   // index of abundance with stock data, in the last 10 years
  init_3darray obs_I_coast_stock(1,stock,1,tstep,fsyear,lsyear) // index of abundance with stock data, in the last 10 years
   
  //spatial observed proportions at age for the survey
  init_3darray obs_Ip_bay(1,tstep,fryear,lryear,fage,lage)
  init_3darray obs_Ip_coast(1,tstep,fryear,lryear,fage,lage)
  init_4darray obs_Ip_bay_stock(1,stock,1,tstep,fsyear,lsyear,fage,lage) //for surveys in the Bay
  init_4darray obs_Ip_coast_stock(1,stock,1,tstep,fsyear,lsyear,fage,lage) //for surveys in the coast
  
  
  //spatial observed CV for the survey
  init_matrix obs_I_CV_bay(1,tstep,fryear,lryear)//using same CV for both timesteps
  init_matrix obs_I_CV_coast(1,tstep,fryear,lryear)//using same CV for both timesteps
  init_3darray obs_I_CV_bay_stock(1,stock,1,tstep,fsyear,lsyear)//using same CV for both timesteps
  init_3darray obs_I_CV_coast_stock(1,stock,1,tstep,fsyear,lsyear)//using same CV for both timesteps
  ...
  init_vector true_Ntot(fdyear,ldyear) //storing the true values of Jan 1 abundance
  init_matrix true_Ntot_stock(1,stock,fdyear,ldyear) //storying the true values of stock specific jan 1 abundance
  init_vector true_biomass(fdyear,ldyear) //storing the true values of Jan1 biomas
  init_vector true_SSB(fdyear,ldyear) //storing the true values of Jan 1 SSB
  //init_vector true_F_year(fmyear,lmyear) //storing true values for total F every year
  init_vector true_F_spr(1,stock) //true F that yields 40% spr
  init_number true_annual_F //f that yield 40% for annual, to compare to FAA
  init_matrix true_F(fdyear,ldyear,fage,lage) //stroing true values of Jan 1 F for each age
  ...
  3darray C_var(1,region,1,tstep,fryear,lryear)  //log-scale variance of catch
  4darray C_var_stock(1,stock,1,region,1,tstep,fsyear,lsyear)  //log-scale variance of catch
 
  //spatial variance
  matrix I_var_bay(1,tstep,fryear,lryear) //log-scale variance of index of abundancefor ches bay
  matrix I_var_coast(1,tstep,fryear,lryear) //log scale variance of atl coast
  3darray I_var_bay_stock(1,stock,1,tstep,fsyear,lsyear) //log-scale variance of index of abundancefor ches bay
  3darray I_var_coast_stock(1,stock,1,tstep,fsyear,lsyear) //log scale variance of atl coast
  ...
  number G //looping variable for spr model
 ...
 LOCAL_CALCS
  if(test!=12345)
  {
    ...
    cout << "obs I coast stock" << endl << obs_I_coast_stock << endl;
    cout << "obs I bay stock" << endl << obs_I_bay_stock << endl;
    cout << "obs prop coast surv" << endl << obs_Ip_coast_stock << endl;
    cout << "obs prop bay surv" << endl << obs_Ip_bay_stock << endl;
    cout << "obs cv coast" << endl << obs_I_CV_coast_stock << endl;
    cout << "obs cv bay" << endl << obs_I_CV_bay_stock << endl;
    ...
    cout << "ESS C" << endl << ESS_C << endl;
    cout << "ESS S coast" << endl << ESS_I_coast << endl;
    cout << "ESS S bay" << endl << ESS_I_bay << endl;
    cout << "true n" << endl << true_Ntot << endl;
    cout << "true bio" << endl << true_biomass << endl;
    cout << "true ssb" << endl << true_SSB << endl;
  ...
  for(s=1;s<=stock;s++)
  {
    for(r=1;r<=region;r++)
    {
      ...
      //cout << lmyear << endl;
      //cout << C_var(r,t) << endl;
      C_var_stock(s,r,t)=square(obs_CV_stock(s,r,t));//calculate variance from standard deviations
  ...
  for(t=1;t<=tstep;t++)
  {
    I_var_coast(t)=square(obs_I_CV_coast(t));
    I_var_bay(t)=square(obs_I_CV_bay(t));
    for(s=1;s<=stock;s++)
    {
      I_var_coast_stock(s,t)=square(obs_I_CV_coast_stock(s,t));
      I_var_bay_stock(s,t)=square(obs_I_CV_bay_stock(s,t));
    }
  }
  //cout << "end Ivar loop" << endl;

  I_var_age1_bay=square(obs_I_age1_CV_bay);
  I_var_age1_coast=square(obs_I_age1_CV_coast);
  
  I_var_yoy_coast=square(obs_yoy_CV_coast);
  I_var_yoy_bay=square(obs_yoy_CV_bay);
  ...
  */
  ...
  for(s=1;s<=stock;s++)
  {
     for(ts=1;ts<=tstep;ts++)
     {
       //cout << "obs catch" << endl << "s=" << s << " r=" << r << " ts= " << ts << endl << obs_C(s,r,ts) << endl;
       cout << "obs I stock coast" << endl << "s=" << s <<  " ts= " << ts << endl << obs_I_coast_stock(s,ts) << endl;
       cout << "obs I stock bay" << endl << "s=" << s <<  " ts= " << ts << endl << obs_I_bay_stock(s,ts) << endl;
     }
  }
  ...
  for(s=1;s<=stock;s++)
  {
   for(r=1;r<=region;r++)
   {
     for(ts=1;ts<=tstep;ts++)
     {
        //cout << "obs prop age catch" << endl << "s=" << s << " r=" << r << " ts= " << ts << endl << obs_Cp(s,r,ts) << endl;
        cout << "obs prop age coast" << endl << "s=" << s << " ts= " << ts << endl << obs_Ip_coast_stock(s,ts) << endl;
        cout << "obs prop age bay" << endl << "s=" << s << " ts= " << ts << endl << obs_Ip_bay_stock(s,ts) << endl;
     }
   }
  }
  exit(1);
...
PARAMETER_SECTION
  ...
  init_bounded_number log_a_sf1(-2,2) //log of slope of afsel
  init_bounded_number log_a_sf2(0,5) // log age of 50% afsel
  ...
  matrix est_Ntot_stock(1,stock,fmyear,lmyear)//total stock specific abundance Jan 1
  ...
  5darray sigma2_Cp_stock(1,stock,1,region,1,tstep,fmyear,lmyear,fage,lage) //variance of estimate proportions at age in the catch
  ...
  //stock data for bay survey
  4darray est_I_age_bay_stock(1,stock,1,tstep,fmyear,lmyear,fage,lage)//estimated catch at age, ches bay
  4darray est_I_age_bay_err(1,stock,1,tstep,fmyear,lmyear,fage,lage) //estimated I for the bay survey, with aging error applied
  4darray est_Ip_bay_stock(1,stock,1,tstep,fmyear,lmyear,fage,lage)//estimate proportions at age, ches bay
  3darray est_I_bay_stock(1,stock,1,tstep,fmyear,lmyear)//estimated aggregate index of abundance for ches bay survey
  4darray sigma2_Ip_bay_stock(1,stock,1,tstep,fmyear,lmyear,fage,lage)//variance of est prop at age 
  ...
  //stock data for coast survey
  4darray est_I_age_coast_stock(1,stock,1,tstep,fmyear,lmyear,fage,lage) //estimated catch at age, atl coast
  4darray est_I_age_coast_err(1,stock,1,tstep,fmyear,lmyear,fage,lage) //estimated catch at age,with aging error
  4darray est_Ip_coast_stock(1,stock,1,tstep,fmyear,lmyear,fage,lage)//estimated proportions at age,atl coast
  3darray est_I_coast_stock(1,stock,1,tstep,fmyear,lmyear)//estimated aggregate index of abundance for atl coast
  4darray sigma2_Ip_coast_stock(1,stock,1,tstep,fmyear,lmyear,fage,lage)//variance of est prop at age atl coast
  ...
  vector est_I_yoy_bay(fmyear,lmyear)     //estimated istripedbassstripedbassndex of abundance for bay YOY
  ...
  number est_G //numer for weighted G values
  3darray F_bar(1,region,1,tstep,fage,lage)
  matrix F_bar_stock(1,stock,fage,lage) //stock specific fs at age that achieve 40% spr
  vector est_F_spr(1,stock)
  number est_annual_F
  ...
  3darray Lcatch_s(1,stock,1,region,1,tstep)
  matrix Lcatch_r(1,region,1,tstep)
  3darray Lcatchagecomp_s(1,stock,1,region,1,tstep)
  matrix Lcatchagecomp_r(1,region,1,tstep)

  vector Lindex_coast_r(1,tstep)
  vector Lindexagecomp_coast_r(1,tstep)
  matrix Lindex_coast_s(1,stock,1,tstep)
  matrix Lindexagecomp_coast_s(1,stock,1,tstep)
  vector Lindex_bay_r(1,tstep)
  vector Lindexagecomp_bay_r(1,tstep)
  matrix Lindex_bay_s(1,stock,1,tstep)
  matrix Lindexagecomp_bay_s(1,stock,1,tstep)
 ...
 */
  ...
  */
  ...
  //calc total stock specificabundance
  ...
  for(y=fmyear;y<=lmyear;y++)
  {
    for(r=1;r<=region;r++)
    {
      for(t=1;t<=tstep;t++)
      {
       ...
       { 
        //ts=1;
        est_F(y,a)+=(F(r,t,y,a)*N_region(r,t,y,a))/(N_region(1,t,y,a)+N_region(2,t,y,a));//
       }
      }//close tstep
    }//close year
  }//clsoe region
  ...
  */
  ...
  { 
	...
	for(s=1;s<=stock;s++)
        {
          //cout << s << endl;
	  est_Cp(s,r,t,y)=sum(est_C_age_err(s,r,t,y)); //sum catch ove age
          est_Cp(s,r,t,y)(fage,lage)/=est_C(s,r,t,y); //divide by total catch to get proportions at age
        }
        for(a=fage;a<=lage;a++)
        {
    ...
    for(r=1;r<=region;r++)
    {
      ...
      for(s=1;s<=stock;s++)
      {
        ...
        { 
          for(a=fage;a<=lage;a++)
          {
           //sigma2_Cp(r,t,y,a)=((1.-est_region_Cp(r,t,y,a))*est_region_Cp(r,t,y,a)+(d/double(lage-fage+1)))/ESS_C(r);
           sigma2_Cp_stock(s,r,t,y,a)=((1.-est_Cp(s,r,t,y,a))*est_Cp(s,r,t,y,a)+(d/double(lage-fage+1)))/ESS_C(r,t);
          }
         }//close timestep loop for sigma2cp
       }//close stock loop
       for(t=1;t<=tstep;t++)
       { 
         for(a=fage;a<=lage;a++)
...
FUNCTION calculate_survey_catch
  ...
  for(y=fmyear;y<=lmyear;y++)
  {
    for(ts=1;ts<=tstep;ts++)
    {
      for(s=1;s<=stock;s++)
      {
        est_I_age_bay_stock(s,ts,y)=q_bay(ts)*elem_prod(N(s,1,ts,y),ssel_bay(ts));//region1
        est_I_age_bay_err(s,ts,y)=mod_age_err_a*est_I_age_bay_stock(s,ts,y);
      }
    }//close ts loop
  }//closing year loop
  ...
  for(s=1;s<=stock;s++)
  {
    for(ts=1;ts<=tstep;ts++)
    {
      est_I_bay_stock(s,ts)=rowsum(est_I_age_bay_err(s,ts));
    }
  }
  ...
  {//calculate proportions at age in the survey catch
    for(s=1;s<=stock;s++)
    {
      for(ts=1;ts<=tstep;ts++)
      {
        est_Ip_bay_stock(s,ts,y)=est_I_age_bay_err(s,ts,y)/est_I_bay_stock(s,ts,y);
        for(a=fage;a<=lage;a++) //can still use age ranges for each survey
        {
          sigma2_Ip_bay_stock(s,ts,y,a)=((1.-est_Ip_bay_stock(s,ts,y,a))*est_Ip_bay_stock(s,ts,y,a)+0.1/double(lage-fage+1))/ESS_I_bay(ts);
        }//close age loop
      }// close timestep loop
    }// close stock loop
  }//close year loop
  ...
  for(y=fmyear;y<=lmyear;y++)
  {
    for(s=1;s<=stock;s++)
    {
      for(ts=1;ts<=tstep;ts++)
      {
        est_I_age_coast_stock(s,ts,y)=q_coast(ts)*elem_prod(N(s,2,ts,y),ssel_coast(ts));//region2
        est_I_age_coast_err(s,ts,y)=mod_age_err_a*est_I_age_coast_stock(s,ts,y);
      }//close ts loop
    }//close stock loop
  }//closing year loop
  ...
  for(s=1;s<=stock;s++)
  {
    for(ts=1;ts<=tstep;ts++)
    {
    est_I_coast_stock(s,ts)=rowsum(est_I_age_coast_err(s,ts));
    }
  }
  ...
  {//calculate proportions at age in the survey catch
    for(s=1;s<=stock;s++)
    {
      for(ts=1;ts<=tstep;ts++)
      {
        est_Ip_coast_stock(s,ts,y)=est_I_age_coast_err(s,ts,y)/est_I_coast_stock(s,ts,y);
        for(a=fage;a<=lage;a++) //can still use age ranges for each survey
        {
          sigma2_Ip_coast_stock(s,ts,y,a)=((1.-est_Ip_coast_stock(s,ts,y,a))*est_Ip_coast_stock(s,ts,y,a)+0.1/double(lage-fage+1))/ESS_I_coast(ts);
        }//close age loop
      } // close timestep loop
    }//close stock loop
  }//close year loop
  //cout << est_I_coast << endl;
  //exit(1);


  //aggregate survey data without stock state
  for(ts=1;ts<=tstep;ts++)
  {
    for(y=fmyear;y<=lmyear;y++)
    {
      est_I_bay(ts,y)=est_I_bay_stock(1,ts,y)+est_I_bay_stock(2,ts,y);
      est_I_coast(ts,y)=est_I_coast_stock(1,ts,y)+est_I_coast_stock(2,ts,y);

      //est_I age age
      est_I_age_coast(ts,y)=est_I_age_coast_err(1,ts,y)+est_I_age_coast_err(2,ts,y);
      est_I_age_bay(ts,y)=est_I_age_bay_err(1,ts,y)+est_I_age_bay_err(2,ts,y);
      //proportions at age
      est_Ip_bay(ts,y)=est_I_age_bay(ts,y)/est_I_bay(ts,y);
      est_Ip_coast(ts,y)=est_I_age_coast(ts,y)/est_I_coast(ts,y);
      for(a=fage;a<=lage;a++)
      {
  ...
  //cout << "est ip bay region" << endl << est_Ip_bay << endl;
...
FUNCTION evaluate_likelihood
  ...
  //without stock structure, years 1-20
  for(ts=1;ts<=tstep;ts++)
  {
    //cout << "t=" << t << " " << tstep << endl;
    for(r=1;r<=region;r++)
    {
      //cout << "r=" << r << endl;
       ...
       Lcatch_r(r,ts)=lognorm_negLL(obs_C(r,ts),est_region_C(r,ts),C_var(r,ts),fryear,lryear);
  ...
  for(s=1;s<=stock;s++)
  {
    for(ts=1;ts<=tstep;ts++)
    {
      ...
      for(r=1;r<=region;r++)
      {
        //cout << "r=" << r << endl;
         //Lcatch(r,t)=lognorm_negLL(obs_C(r,t),est_region_C(r,t),C_var(r,t),fmyear,lmyear);
         Lcatch_s(s,r,ts)=lognorm_negLL(obs_C_stock(s,r,ts),est_C(s,r,ts),C_var_stock(s,r,ts),fsyear,lsyear);
       //cout << est_region_C(r,t) << endl;
      }//close region loop
    }//close time step loop
  } //close stock loop
  ...
  for(s=1;s<=stock;s++)
  {
    ...
    {  
      for(r=1;r<=region;r++)
      {
        ...
        Lcatchagecomp_s(s,r,ts)=multinom_negLL(obs_Cp_stock(s,r,ts),est_Cp(s,r,ts),sigma2_Cp_stock(s,r,ts),fage,lage,fsyear,lsyear);//multinomial for proportions
      }//close region loop
    }//close timestep loop
  }//close region loop
  ...
  {  
    ...
    {
        //Lcatchagecomp(r,ts)=multinom_negLL(obs_Cp(r,ts),est_region_Cp(r,ts),sigma2_Cp(r,ts),fage,lage,fmyear,lmyear);//multinomial for proportions
      Lcatchagecomp_r(r,ts)=multinom_negLL(obs_Cp(r,ts),est_region_Cp(r,ts),sigma2_Cp(r,ts),fage,lage,fryear,lryear);//multinomial for proportions
    }//close region loop
  }//close timestep loop

  //cout << Lcatchagecomp_r<< endl;
  //cout << Lcatchagecomp_s<< endl;
  //exit(1);
  //lognormal likelkihood for indices of abundance

  //survey likelihoods without stock, years 1-20
  for(ts=1;ts<=tstep;ts++)
  {
    //COAST
    Lindex_coast_r(ts)=lognorm_negLL(obs_I_coast(ts),est_I_coast(ts),I_var_coast(ts),fryear,lryear);
    Lindexagecomp_coast_r(ts)=multinom_negLL(obs_Ip_coast(ts),est_Ip_coast(ts),sigma2_Ip_coast(ts),fage,lage,fryear,lryear);
    //CHES BAY
    Lindex_bay_r(ts)=lognorm_negLL(obs_I_bay(ts),est_I_bay(ts),I_var_bay(ts),fryear,lryear);
    Lindexagecomp_bay_r(ts)=multinom_negLL(obs_Ip_bay(ts),est_Ip_bay(ts),sigma2_Ip_bay(ts),fage,lage,fryear,lryear);
  }
  //cout << "obs I bay" << endl << obs_I_bay << endl;
  //cout << "obs I bay stock" << endl << obs_I_bay_stock << endl;
  //cout << "est I bay" << endl << est_I_bay << endl;
  //cout << "est I bay stock" << endl << est_I_bay_stock << endl;
  //cout << "i var bay" << endl << I_var_bay << endl;


  //with stock data, year 21-30
  for(s=1;s<=stock;s++)
  {
    for(ts=1;ts<=tstep;ts++)
    {
      //COAST
      Lindex_coast_s(s,ts)=lognorm_negLL(obs_I_coast_stock(s,ts),est_I_coast_stock(s,ts),I_var_coast_stock(s,ts),fsyear,lsyear);
      Lindexagecomp_coast_s(s,ts)=multinom_negLL(obs_Ip_coast_stock(s,ts),est_Ip_coast_stock(s,ts),sigma2_Ip_coast_stock(s,ts),fage,lage,fsyear,lsyear);//change AGEEE
      //CHES BAY
      Lindex_bay_s(s,ts)=lognorm_negLL(obs_I_bay_stock(s,ts),est_I_bay_stock(s,ts),I_var_bay_stock(s,ts),fsyear,lsyear);
      Lindexagecomp_bay_s(s,ts)=multinom_negLL(obs_Ip_bay_stock(s,ts),est_Ip_bay_stock(s,ts),sigma2_Ip_bay_stock(s,ts),fage,lage,fsyear,lsyear);
    }
  }

  //cout << "index bay r" << endl << Lindex_bay_r << endl;
  //cout << "index bay s" << endl << Lindex_bay_s << endl;
  //cout << "index coast r" << endl << Lindex_coast_r << endl;
  //cout <<  "nidex coast s" << endl << Lindex_coast_s << endl;
  //cout << "age comp coast r" << endl << Lindexagecomp_coast_r << endl;
  //cout <<  "age comp coast s" << endl << Lindexagecomp_coast_s << endl;
  //cout << "age comp bay r" << endl << Lindexagecomp_bay_r << endl;
  //cout <<  "age comp bay s" << endl << Lindexagecomp_bay_s << endl;
  //exit(1);
  ...
  //cout << "pen_prop" << endl << pen_prop << endl;
  ...
  neg_LL=sum(Lcatch_r)+sum(Lcatch_s)+sum(Lcatchagecomp_r)+sum(Lcatchagecomp_s)+
          sum(Lindex_bay_r)+sum(Lindex_bay_s)+sum(Lindex_coast_r)+sum(Lindex_coast_s)+
          sum(Lindexagecomp_bay_r)+sum(Lindexagecomp_bay_s)+sum(Lindexagecomp_coast_r)+sum(Lindexagecomp_coast_s)+
          Lage1index_coast+Lage1index_bay+Lyoyindex_bay+Lyoyindex_coast+pen_F+pen_prop+pen_prop_bay+pen_cb_sel+
          pen_rdev+pen_fdev;
  /*
  cout << sum(Lcatch_r) << sum(Lcatch_s) << sum(Lcatchagecomp_r) << sum(Lcatchagecomp_s) << endl <<
          sum(Lindex_bay_r) << sum(Lindex_bay_s) << sum(Lindex_coast_r) << sum(Lindex_coast_s) << endl <<
          sum(Lindexagecomp_bay_r) << sum(Lindexagecomp_bay_s) << sum(Lindexagecomp_coast_r) << sum(Lindexagecomp_coast_s) << endl <<
          Lage1index_coast << Lage1index_bay << Lyoyindex_bay << Lyoyindex_coast << pen_F << pen_prop << pen_prop_bay <<pen_cb_sel << endl << 
          pen_rdev << pen_fdev << endl << endl;
  cout << "neg ll " << endl << neg_LL << endl;
  */
  //exit(1);
...
REPORT_SECTION
  ...
  report << "observed index coast" << endl << obs_I_coast_stock << endl;
  report << "observed index bay" << endl << obs_I_bay_stock <<endl;
  //report << "estimated index coast" << endl << est_I_coast << endl;
  report << "estimated index bay" << endl << est_I_bay_stock << endl;
  report << "estimated index coast" << endl << est_I_coast_stock << endl;
  ...
  {
    catout << "year timestep stock region logobs logpred " << endl;
    for(y=fmyear;y<=lmyear;y++)
    {
      for(t=1;t<=tstep;t++)
      {
       for(r=1;r<=region;r++)
       {
         for(s=1;s<=stock;s++)
         {
           catout << y << " " << t << " " << s << " " << r << " "  << log(obs_C(s,r,t,y)+0.0001) << " " << log(est_C(s,r,t,y)+0.00001) << endl;
         }
        }//close region
      }//close tstep
    }//close yr
    ...
    {
        ioaout << "agegroup survey year stock  timestep obsioa predioa" << endl;
        for(y=fmyear;y<=lmyear;y++)
        {
          for(ts=1;ts<=tstep;ts++)
          {
            for(s=1;s<=stock;s++)
            {
              ioaout << "1-15" << " " << "bay" << " " << y << " " << s << " " << ts << " "<< log(obs_I_bay_stock(s,ts,y)+0.00001) << " " << log(est_I_bay_stock(s,ts,y)+0.00001) << endl;
              ioaout << "1-15" <<  " " << "coast" << " " << y << " " << s << " " << ts << " "<< log(obs_I_coast_stock(s,ts,y)+0.00001) << " " << log(est_I_coast_stock(s,ts,y)+0.00001) << endl;
          }//close timestep
       }//close year
     }//close ofstream
   }
   ...
   {
     obscaa << " region timestep year stock age obscaa estcaa obsminusest standardresid" << endl;
     for(y=fmyear;y<=lmyear;y++)
     {
       for(t=1;t<=tstep;t++)
       {
           for(a=fage;a<=lage;a++)
           {
             for(r=1;r<=region;r++)
             {
               for(s=1;s<=stock;s++)
               {
                 obscaa << " " << r << " " << t << " " << y << " " << s << " " << a << " " <<  log(obs_Cp(s,r,t,y,a)+0.001) << " " << log(est_Cp(s,r,t,y,a)+0.001) << " " << obs_Cp(s,r,t,y,a)-est_Cp(s,r,t,y,a) << " " << (obs_Cp(s,r,t,y,a)-est_Cp(s,r,t,y,a))/sqrt(C_var(s,r,t,y)) << endl;
               }
             }//close region
           }//close age      
     }//close tstep
   ...
   {
     obssaa << "agegroup survey year timestep stock age obssaa estsaa obsminusest standardresid" << endl;
     for(y=fmyear;y<=lmyear;y++)
     {
       for(a=fage;a<=lage;a++)
       {
         for(ts=1;ts<=tstep;ts++)
         {
           for(s=1;s<=stock;s++)
           {
            if(obs_Ip_bay_stock(s,ts,y,a)!=-99) obssaa << "1-15" << " " << "bay"  << " " << y << " " << ts << " " << s << " " << a << " " <<  log(obs_Ip_bay_stock(s,ts,y,a)+0.001) << " " <<  log(est_Ip_bay_stock(s,ts,y,a)+0.001) << " " << obs_Ip_bay_stock(s,ts,y,a)-est_Ip_bay_stock(s,ts,y,a) << " " << (obs_Ip_bay_stock(s,ts,y,a)-est_Ip_bay_stock(s,ts,y,a))/sqrt(I_var_bay_stock(s,ts,y)) << endl;
            if(obs_Ip_coast_stock(s,ts,y,a)!=-99) obssaa << "1-15" << " " << "coast"  << " " << y << " " << ts << " " << s << " " << a << " " <<  log(obs_Ip_coast_stock(s,ts,y,a)+0.001) << " " <<  log(est_Ip_coast_stock(s,ts,y,a)+0.001) << " " << obs_Ip_coast_stock(s,ts,y,a)-est_Ip_coast_stock(s,ts,y,a) << " " << (obs_Ip_coast_stock(s,ts,y,a)-est_Ip_coast_stock(s,ts,y,a))/sqrt(I_var_coast_stock(s,ts,y)) << endl;
           }
         }//close tstep
       }//close age
     }//close year
     ...
     for(r=1;r<=region;r++)
     {
       for(y=fmyear;y<=lmyear;y++)
       {
         for(t=1;t<=tstep;t++)
         {
           for(a=fage;a<=lage;a++)
           {
             fselout << r  << " " << t << " " << y << " " << a << " " << log(fsel(r,t,y,a)+0.0001) << endl;
           }//age
         }//close tstep
       }//clse year
     }//closeregion
   ...
   ofstream sselout("ssel.txt");//survey selectivty
   {
     ...
     for(a=fage;a<=lage;a++)
     {
       for(t=1;t<=tstep;t++)
       {
         sselout << "1-15 " << "bay " << " " << t << " "<< a << " " << log(ssel_bay(t,a)+0.0001) << endl;
         sselout << "1-15 " << "coast " << " " << t << " "<< a << " " << log(ssel_coast(t,a)+0.0001) << endl;
         }//close tstep
     }//close a age
   }//close ofstream
   ...
   ofstream obsage1("age1.txt");
   {
     ...
     for(y=fmyear;y<=lmyear;y++)
     {
       if(obs_I_age1_coast(y)!=-99) obsage1 << "coast" << " "  << y << " " << "2" << " "  <<  log(obs_I_age1_coast(y)+0.001) << " " <<  log(est_I_age1_coast(y)+0.001) << endl;
       if(obs_I_age1_bay(y)!=-99) obsage1 << "bay"  << " " << y << " " << "2" << " " << log(obs_I_age1_bay(y)+0.001) << " " <<  log(est_I_age1_bay(y)+0.001) << endl;
     }
   }
   ...
   ofstream obsyoy("yoy.txt");
   {
     ...
     for(y=fmyear;y<=lmyear;y++)
     {
      if(obs_I_yoy_coast(y)!=-99) obsyoy << "coast" << " "  << y << " " <<  log(obs_I_yoy_coast(y)+0.001) << " " <<  log(est_I_yoy_coast(y)+0.001) << endl;
      if(obs_I_yoy_bay(y)!=-99) obsyoy  << "bay" << " "  << y << " " <<  log(obs_I_yoy_bay(y)+0.001) << " " <<  log(est_I_yoy_bay(y)+0.001) << endl;
     }//close year loop
   }//close of stream
    ...
    ofstream like("lik.txt");
   {
     ...
     like << " " << "sumLcatch_bay" << " " << sum(Lcatch(1,1))+sum(Lcatch(1,2)) << endl;//adding for both timesteps
     like << " " << "sumLcatch_coast" << " " << sum(Lcatch(2,1))+sum(Lcatch(2,2)) << endl;
     like << " " << "sumLcatchagecomp_bay" << " " << sum(Lcatchagecomp(1,1))+sum(Lcatchagecomp(1,2)) << endl;
     like << " " << "sumLcatchagecomp_coast" << " " << sum(Lcatchagecomp(2,1))+sum(Lcatchagecomp(2,2)) << endl;   
...
FINAL_SECTION
  ...
  for(G=0;G<=150;G++)  //looping through f's 
  {
   ...
   for(s=1;s<=stock;s++)
   {
     for(r=1;r<=region;r++)
     {
       //for(a=fage;a<=lage;a++)
       //{
         Nf(s,r,1,fmyear)=migrate(Nf_stock(s,1,fmyear),prop(s,1,r));
       //}//close age loop
     }//close region loop
   } //close stock loop
   ...
   //now to the reamianing years frmo year2-100 year
   for(y=2;y<=years;y++)
   {
     for(s=1;s<=stock;s++)
     {
       ...
       for(a=2;a<=lage;a++)
       {
         Nf_stock(s,1,y,a)=Nf(s,1,2,y-1,a-1)*mfexp(-Zf(1,2,a-1))+Nf(s,2,2,y-1,a-1)*mfexp(-Zf(2,2,a-1));
       }//close age loop
       Nf_stock(s,1,y,lage)=Nf_stock(s,1,y,lage)+Nf(s,1,2,y-1,lage)*mfexp(-Zf(1,2,lage))+Nf(s,2,2,y-1,lage)*mfexp(-Zf(2,2,lage));
   ...
   for(r=1;r<=region;r++)
   {
     //for(ts=1;ts<=tstep;ts++)
     //{
       SSBf(1,G)+=sum(elem_prod(Nf(1,r,1,100),elem_prod(sex,elem_prod(rw_age(30),m_age)))); //stock =1, ts=1, 100=last year of simulated pop
       SSBf(2,G)+=sum(elem_prod(Nf(2,r,1,100),elem_prod(sex,elem_prod(rw_age(30),m_age)))); //stock =2, ts=1, 100=last year of simulated pop
       //cout << SSBf(1,G) << endl;
     // }  //close tstep loop
   } //close region loop
  for(s=1;s<=stock;s++)
  ...
  est_G=est_G_40(1)*R_eq(1)+est_G_40(2)*R_eq(2);
  ...
  est_annual_F+=F_bar_stock(1,8)*R_eq(1)+F_bar_stock(2,8)*R_eq(2); //setting annual value as age 8 reference weighted by recruitment
  ...
  ofstream ofs("sim_results.txt",ios::app);
  {
   for(y=fmyear;y<=lmyear;y++)
   {
     //ofs << " " << sim_num << " " << est_F(y,8) << " " << elem_div((est_F(y,8)-true_F(y,8)),true_F(y,8)) << " " <<  est_Ntot(y) << " " << elem_div((est_Ntot(y)-true_Ntot(y)),true_Ntot(y))
     //  << " " <<  est_biomass(y)<< " " << elem_div((est_biomass(y)-true_biomass(y)),true_biomass(y)) << " " << est_SSB(y) << " " << elem_div((est_SSB(y)-true_SSB(y)),true_SSB(y)) << " " << objective_function_value::pobjfun->gmax<<endl;
